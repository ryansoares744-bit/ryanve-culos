window.RVDADOS = (function(){
// Ryan Veículos — dados compartilhados (site público + painel)
const KEY = 'rv_db_v2';
const SESS = 'rv_sessao';

function slugify(s){return s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}
function fmtPreco(n){return n?'R$ '+Number(n).toLocaleString('pt-BR'):'Consulte';}
function fmtKm(n){return Number(n||0).toLocaleString('pt-BR')+' km';}
function tituloDe(v){return v.marca+' '+v.modelo;}
function slugDe(v){return slugify(v.marca+'-'+v.modelo+'-'+v.versao+'-'+v.anoMod);}
function waLink(cfg,msg){return 'https://wa.me/'+(cfg.whatsappNum||'5511995399892')+'?text='+encodeURIComponent(msg);}
...
function resetDB(){localStorage.removeItem(KEY);return loadDB();}

function addContato(db,c){
  db.contatos.unshift(Object.assign({id:Date.now(),data:Date.now(),statusAt:'novo',obs:''},c));
  saveDB(db);
}

function getSessao(){try{return JSON.parse(localStorage.getItem(SESS));}catch(e){return null;}}
function setSessao(s){if(s)localStorage.setItem(SESS,JSON.stringify(s));else localStorage.removeItem(SESS);}

// Compressão de imagem: retorna dataURL JPEG máx 1280px
...
const STATUS_LABEL={disponivel:'Disponível',reservado:'Reservado',vendido:'Vendido',oculto:'Oculto'};
const STATUS_COR={disponivel:'#1B9E4B',reservado:'#C77B00',vendido:'#B3261E',oculto:'#666'};
const AT_LABEL={novo:'Novo contato',atendimento:'Em atendimento',negociacao:'Negociação',visita:'Visita agendada',venda:'Venda concluída',sem:'Sem interesse'};

return {slugify,fmtPreco,fmtKm,tituloDe,slugDe,waLink,seedDB,loadDB,saveDB,resetDB,addContato,getSessao,setSessao,comprimirImagem,STATUS_LABEL,STATUS_COR,AT_LABEL};
})();